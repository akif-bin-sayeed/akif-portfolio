...
function normalize(raw){ const d=structuredClone(window.DEFAULT_SITE_CONTENT); const c=deepMerge(d,raw||{}); if(typeof c.about==='string') c.about={paragraphs:[c.about], stats:d.about.stats}; c.about.paragraphs=arr(c.about.paragraphs); c.about.stats=arr(c.about.stats); return c; }
...
async function init(){
  ...
  $('#seedBtn').onclick = async()=>{ content=normalize(window.DEFAULT_SITE_CONTENT); await saveContent(); renderTab(); showNotice('Starter demo content has been seeded.'); };
...
function renderProfile(){
  ...
  $('#logoFile').onchange=async e=>{ const file=e.target.files[0]; if(!file)return; const url=await uploadFile('profile',file); if(url){content.site.logoImage=url; renderProfile(); showNotice('Circle logo uploaded. Click Save content.');}};
  ...
  $('#profileFile').onchange=async e=>{ const file=e.target.files[0]; if(!file)return; const url=await uploadFile('profile',file); if(url){content.hero.profileImage=url; renderProfile(); showNotice('Profile image uploaded. Click Save content.');}};
