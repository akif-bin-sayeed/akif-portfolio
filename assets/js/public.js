...
function renderNav(){
  ...
  $('#navLinks').innerHTML = ['about','experience','projects','skills','certificates','gallery','posts','contact'].map(id => `<a href="#${id}">${escapeHTML(map[id] || id)}</a>`).join('') + '<button id="themeToggle" class="theme-toggle" aria-label="Toggle theme">◐</button>';
...
async function renderGallery(){
  ...
  $('#gallery').innerHTML = renderSectionHead('gallery') + (rows.length ? `<div class="masonry">${rows.map(g=>`<figure class="masonry-item reveal"><img src="${escapeHTML(g.image_url)}" alt="${escapeHTML(g.title || 'Gallery image')}" loading="lazy"><figcaption class="masonry-caption"><strong>${escapeHTML(g.title || '')}</strong><p>${escapeHTML(g.caption || '')}</p></figcaption></figure>`).join('')}</div>` : `<div class="card reveal"><p>Gallery is ready. Upload photos from the private app.</p></div>`);
}
async function renderCertificates(){
  const rows = await fetchRows('certificates');
  $('#certificates').innerHTML = renderSectionHead('certificates') + (rows.length ? `<div class="grid cards-3">${rows.map(c=>`<article class="card reveal">${c.image_url?`<img class="cert-img" src="${escapeHTML(c.image_url)}" alt="${escapeHTML(c.title)}" loading="lazy">`:''}<div class="mini">${escapeHTML(c.year || '')}</div><h3>${escapeHTML(c.title)}</h3><p>${escapeHTML(c.issuer || '')}</p></article>`).join('')}</div>` : `<div class="card reveal"><p>Certificate corner is ready. Add certificates from the private app.</p></div>`);
}
...
function toggleTheme(){ document.body.classList.toggle('light'); localStorage.setItem('portfolioTheme', document.body.classList.contains('light')?'light':'dark'); }
...
if(localStorage.getItem('portfolioTheme')==='light') document.body.classList.add('light');
loadData();
